...
void bucketSort(float a[], int n){ //elements in array are floating point type those differs between [0, 1).
	vector<float> b[n]; //vector of bucket array. each bucket can take n elements of same type of array.

	for (int i = 0; i < n; i++) {
		int bi = n * a[i]; //indexes for buckets are casted to integers differs between [0, n).
